...
<script runat="server">
    ...
    private void RetrieveExistingPlayer()
    {
        ...
        try
        {
            ...
            string strQuery = "SELECT PlayerName, PlayerManufacturerID, PlayerCost, PlayerStorage FROM Player WHERE PlayerID=@PlayerID; SELECT WPWFFormatID FROM WhatPlaysWhatFormat WHERE WPWFPlayerID = @PlayerID;";
            SqlCommand myCommand = new SqlCommand(strQuery, myConnection);
            myCommand.Parameters.AddWithValue("@PlayerID", Request.QueryString["PlayerID"]);
            ...
            {
                PlayerName.Text = myReader.GetString(myReader.GetOrdinal("PlayerName"));
                ManufacturerList.SelectedValue = myReader.GetInt32(myReader.GetOrdinal("PlayerManufacturerID")).ToString();
                PlayerCost.Text = myReader.GetDecimal(myReader.GetOrdinal("PlayerCost")).ToString();
                PlayerStorage.Text = myReader.GetString(myReader.GetOrdinal("PlayerStorage"));
            }
            ...
            while (myReader.Read() == true)
            {
                foreach (ListItem objFormat in FormatList.Items)
                {
                    if (objFormat.Value == myReader.GetInt32(myReader.GetOrdinal("WPWFFormatID")).ToString())
                    {
