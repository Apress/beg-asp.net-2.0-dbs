...
<script runat="server">
    ...
    protected void DetailsView1_ItemDeleting(object sender, DetailsViewDeleteEventArgs e)
    {
        ...
        {
            // query to execute
            string strQuery = "SELECT COUNT(*) FROM Player WHERE PlayerManufacturerID = ?";
...
<body>
    ...
    <div>
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:OdbcConnectionString %>" ProviderName="<%$ ConnectionStrings:OdbcConnectionString.ProviderName %>" SelectCommand="SELECT ManufacturerID, ManufacturerName FROM Manufacturer ORDER BY ManufacturerName">
        </asp:SqlDataSource>
        <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:OdbcConnectionString %>" ProviderName="<%$ ConnectionStrings:OdbcConnectionString.ProviderName %>" DeleteCommand="DELETE FROM Manufacturer WHERE ManufacturerID = ?" InsertCommand="INSERT INTO Manufacturer (ManufacturerName, ManufacturerCountry, ManufacturerEmail, ManufacturerWebsite) VALUES (?, ?, ?, ?)" SelectCommand="SELECT ManufacturerID, ManufacturerName, ManufacturerCountry, ManufacturerEmail, ManufacturerWebsite FROM Manufacturer WHERE (ManufacturerID = ?)" UpdateCommand="UPDATE Manufacturer SET ManufacturerName = ?, ManufacturerCountry = ?, ManufacturerEmail = ?, ManufacturerWebsite = ? WHERE ManufacturerID = ?">
            <DeleteParameters>
